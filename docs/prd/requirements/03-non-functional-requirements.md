...
| NFR-016 † | Security | The system shall maintain an immutable audit log of all security-relevant events: user logins, file uploads, configuration changes, report downloads, role changes, and segment override create/supersede actions, with timestamp and user ID. †Team-proposed — original requirements specify relaxed privacy for anonymous, non-PII waveform data. Authentication scope to be confirmed with OUCRU. | 100% audit coverage for listed event types | Must |
| NFR-016A † | Security | Segment override mutations shall be restricted to `admin` and `reviewer` roles. `researcher` and `readonly` users shall have view-only access to override history and effective classifications. †Team-proposed governance control for adjudication safety. | 0 unauthorized override mutations | Must |
| NFR-016B † | Reliability | If a report was generated before a later segment override, the system shall mark that report as stale within the product experience rather than silently regenerating or mutating the existing report content. | 100% of post-override legacy reports show stale warning | Must |
| NFR-016C † | Maintainability | Feedback-derived learning changes shall follow a staged offline workflow with provenance, evaluation evidence, admin approval, and rollback support; direct online learning from a single override is prohibited. | 0 direct production model/threshold mutations from user overrides | Must |
