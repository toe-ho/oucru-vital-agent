...
| Component | Behavior |
|---|---|
| Waveform Display | Plotly.js line chart. X-axis: time in seconds. Y-axis: amplitude (mV or normalized). Interactive zoom/pan. Highlights the current segment's time range with a shaded overlay. |
...
| Timeline Chart | Embedded Plotly heatmap (same data as dashboard). Read-only in report view. |
