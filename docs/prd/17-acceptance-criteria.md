...
| AC-016 | Token-Encoding Privacy Layer | Token-encoding layer successfully encodes patient identifiers and waveform metadata before any external LLM call; decoding via stored mapping correctly recovers original values | Unit test round-trip encode/decode; integration test asserting no raw patient identifier appears in LLM prompt payloads (checked via mock LLM client call log) | Encode/decode round-trip passes for 100% of test inputs; zero raw patient identifiers present in any captured LLM prompt |
